...
// daily challenge, mock exams, profile and analytics screens.
//
...
class StreakService {
  ...
  List<String> checkBadges(UserStats stats) {
    ...
    if (stats.subjectScores.values.where((s) => s >= 80).length >= 5 &&
        !badges.contains('Subject Master')) {
      badges.add('Subject Master');
    }
    if (stats.subjectAttempts.values.fold(0, (a, b) => a + b) >= 500 &&
        !badges.contains('Marathoner')) {
...
class AppProvider extends ChangeNotifier {
  ...
  Future<void> addXP(int amount) async {
    ...
    _stats = _stats.copyWith(badges: _streak.checkBadges(_stats));
    await _storage.saveUserStats(_stats);
    notifyListeners();
  }
          ...
          ...provider.getAvailableSubjects().map((sub) {
            ...
            return Container(
              ...
              child: ListTile(
                ...
                subtitle: Text('$attempted questions attempted'),
                leading: Icon(
