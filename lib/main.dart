...
class AuthService {
  ...
  Future<String> requestOtpForZetraMail(String zetramail) async {
    ...
    String? authEmail = extractAuthEmail(result);

    // Fallback: stringify the RPC result and attempt to extract an email with regex.
    if (authEmail == null || authEmail.isEmpty) {
      try {
        final raw = jsonEncode(result);
        final emailRegex = RegExp(r'[\w.+-]+@[\w.-]+');
        final matches = emailRegex.allMatches(raw).map((m) => m.group(0)!).toList();
        if (matches.isNotEmpty) {
          // Prefer internal auth emails if possible
          final preferred = matches.firstWhere((s) => s.contains('@auth.') || s.contains('.internal'), orElse: () => matches.first);
          authEmail = preferred;
          debugPrint('[ZetraAuth] extractAuthEmail fallback found: $authEmail');
        }
      } catch (e, st) {
        debugPrint('[ZetraAuth] fallback extraction failed: $e');
        debugPrint('$st');
      }
    }
