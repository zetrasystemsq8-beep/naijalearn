...
//
// UPDATE — Discover & Join / Approval / 3-Admin Cap:
// - New users can now browse open squads (SquadDiscoverScreen) and tap
//   straight in, instead of only being able to join via a typed invite
//   code. "Top Squads" rankings also got a Join button for the same
//   reason.
// - require_approval is now actually wired up end-to-end: joining a
//   squad that requires approval creates a pending request instead of
//   silently doing nothing. The requester sees a "Pending" banner (with
//   Cancel) in place of the join buttons, and squad admins see a
//   Pending Requests section in Members with Approve/Reject actions.
// - Co-leader ("admin") promotions are capped at 3 per squad — enforced
//   both client-side (button disables at the cap) and server-side (the
//   RPC itself rejects a 4th). Squads with 10+ members and fewer than 3
//   admins get a gentle nudge banner suggesting the leader fill the
//   remaining admin slots.
// - Squad settings (name/description/emoji/require-approval) are now
//   editable after creation, leader-only.
// - Every tab's data load now has real error handling (a message + Try
//   Again button) instead of a spinner that spins forever on failure.
// - New: a purely cosmetic/informational "Squad Perks" ladder showing
//   what each squad level represents — client-side only, no new
//   currency or purchase flow invented, since there's no backend for
//   that today.
//
// REQUIRED SUPABASE CHANGES: see the SQL migration provided alongside
// this file. New table: squad_join_requests. New RPCs: squad_discover,
// squad_request_join, squad_get_my_pending_request,
// squad_cancel_join_request, squad_get_pending_requests,
// squad_approve_request, squad_reject_request, squad_update_settings.
// Modified RPC: squad_join_by_code (now returns jsonb {status, squad_id}
// instead of void) and squad_change_role (now enforces the 3-admin cap).
//
...
//   - Real squad "upgrades" (purchasable perks/currency) — the Perks
//     screen added here is an informational level ladder only; wiring
//     up actual purchases would need a currency/store design first.
...
/// Cap on how many co-leaders ("admins") a squad can have. Mirrored
/// server-side in squad_change_role — this constant is for client-side
/// UI (disabling the promote action once reached) only.
const int kSquadMaxAdmins = 3;

/// Member count at which a squad is nudged to fill its admin slots.
...
class SquadService extends ChangeNotifier {
  ...
  /// Join by typed invite code. May join immediately, or create a
  /// pending request if the squad requires approval.
  Future<SquadJoinOutcome> joinByCode(String code) async {
    try {
      ...
      if (status == 'joined') await loadMySquad();
  ...
  /// Same joined-vs-pending contract as joinByCode.
  ...
  /// Open (non-full, discoverable) squads the current user isn't
  /// already a member of — for the "browse and tap to join" flow.
  ...
  /// The current user's own pending join request, if any — used to show
  /// a "Pending approval" state instead of the normal join buttons.
  ...
  /// Pending join requests for a squad — only meaningful for a
  /// leader/co-leader caller; the RPC itself enforces that.
  ...
  /// Now returns an error message (if any) instead of swallowing it —
  /// this is what lets the UI surface the 3-admin-cap rejection.
...
/// =========================================================================
/// DISCOVER SQUADS — browse & tap to join, no code needed
/// =========================================================================
...
class _SquadDiscoverScreenState extends State<SquadDiscoverScreen> {
  ...
  Future<void> _join(Map<String, dynamic> squad) async {
    ...
    Navigator.of(context).pushAndRemoveUntil(
      MaterialPageRoute(builder: (_) => const SquadHomeScreen()),
      (route) => route.isFirst,
    );
  }
...
/// Small reusable "load failed" block used by every tab below.
...
/// =========================================================================
/// SETTINGS — now editable (leader-only), not just invite code + leave
/// =========================================================================
        ...
        children: [
          ...
          if (isLeader) ...[
            ...
            SwitchListTile(
              ...
              subtitle: const Text('New join requests need a leader/co-leader to approve them'),
...
/// =========================================================================
///
/// This is NOT a purchasable-upgrade store — there's no currency/backend
/// for that today. It's a roadmap of what each squad level represents,
/// so members have something to look forward to as the squad's shared
/// XP grows. Safe to extend into a real store later without touching
...
class _SquadRankingsScreenState extends State<SquadRankingsScreen> {
  ...
  String? _error;
  int? _joiningId;
  bool _amInSquad = SquadService.instance.mySquad != null;
